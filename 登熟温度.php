...
$sql = "CREATE VIEW $new_table AS select from_days(to_days(date)) 日付, avg(temp) 平均気温 from $data where date between '2022-08-08' and '2022-09-15' group by from_days(to_days(date)); CREATE VIEW amount_temp AS select sum(平均気温) as 登熟温度 FROM $new_table; select * from amount_temp";
